...
import { entityTiles,type QuakeMap,type QuakePlan } from './engine';
...
export function makeChunk(m: QuakeMap, cx: number, cy: number, context: ReturnType<typeof frameContext>, includeObjects: boolean, motion?:SlideMotion): Chunk {
  const terrain = motion?transitionMesh(m,m,cx,cy):meshChunk({W:m.W,H:m.H,heights:m.heights,columns:new Map()},cx,cy);
