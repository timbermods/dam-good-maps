...
const cliff=new ForceSession(fixture('plain',64));
await complete(cliff,{verb:'quake',settings:{...DEFAULTS.quake,power:80},intent:{path:[{x:0,y:35},{x:63,y:35}],side:1}});
const raised=cliff.map.heights[35*64+50];
await complete(cliff,{verb:'carve',settings:{...DEFAULTS.carve,mode:'aim',defyGravity:true,dry:true,power:85,width:4,wander:0,seed:5},intent:{origin:55*64+50,end:3*64+50}});
assert(cliff.map.heights[35*64+50]<raised);assert.equal(cliff.past.length,2);
const check=new ForceSession(fixture('plain',64));check.import(cliff.export());assert.deepEqual(check.map,cliff.map);
pass('Carve cuts through Quake’s cliff and both forces replay exactly in order');
const tiny=normalize({name:'Small eruption sample',W:8,H:8,maxHeight:22,heights:new Uint8Array(64).fill(6),entities:[],water:{depth:new Float64Array(64),contamination:new Float64Array(64)}});
const sample=new ForceSession(tiny);await complete(sample,{verb:'erupt',settings:{...DEFAULTS.erupt,power:5,seed:7},intent:{origin:36}});
mkdirSync('samples',{recursive:true});writeFileSync('samples/erupt-8.json',JSON.stringify(sample.export())+'\n');
