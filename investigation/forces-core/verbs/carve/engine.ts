import { hardAt } from '../../core/rock';
import { clamp } from "../../core/random";
export { clamp };
import { isPlant, entityTiles, protectedGround } from "../../core/objects";
export { isPlant, entityTiles, protectedGround };
import { plainEntities, objects, modelFor } from "../../core/map";
export { plainEntities, objects, modelFor };
...
export class CarveRun {
  ...
  constructor(input:CarveMap,readonly settings:Settings,intent:Intent,private planning=false) {
    ...
    this.map={...input,...(input.lava?{lava:input.lava.slice()}:{}),heights:input.heights.slice(),entities:plainEntities(input.entities),water:{depth:input.water.depth.slice(),contamination:input.water.contamination.slice()}};
  ...
  private advanceHead() {
    ...
    if(p<.28&&this.original[ahead]-this.bed>4&&this.hard(this.original[ahead],ahead)>.5){this.end('power spent');return;}
  ...
  step():number[] {
    ...
    for(let i=0;i<delta.length;i++)if(delta[i]){
      const d=delta[i];this.map.heights[i]+=d;if(this.map.lava)this.map.lava[i]&=(1<<this.map.heights[i])-1;this.sign[i]=d;changed.push(i);
      if(d<0){
