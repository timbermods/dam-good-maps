import { mixSeed } from "../../core/random";
export { mixSeed };
import type { CarveMap,Settings } from './engine';
export interface Lane {x:number;y:number;width:number}
export interface Knob {x:number;y:number;radius:number}
export const naturalWidth=(power:number)=>2.8+power*.1;
/** Stateless integer mixer; sampling order, frame rate and wall time are irrelevant. */

export class RiverCharacter {
  readonly seed:number;readonly radius:number;readonly intensity:number;readonly wander:number;
  readonly knobs:Knob[]=[];readonly rock:Uint8Array;
  private phase:number;private phase2:number;
  constructor(m:CarveMap,s:Settings,geology:number,origin:number,end?:number){
    this.seed=mixSeed(geology^Math.imul(s.seed??0,0x9e3779b9)^origin^Math.imul(end??0,97));
    this.phase=this.seed/4294967296*Math.PI*2;this.phase2=mixSeed(this.seed)/4294967296*Math.PI*2;
    const width=s.width??naturalWidth(s.power);
    this.radius=width/2;this.wander=(s.wander??35)/100;
    // The same volume concentrated into a slot cuts deeper; a broad override
    // spreads its work. Auto width retains the existing power relationship.
    this.intensity=Math.sqrt(naturalWidth(s.power)/width);
    this.rock=new Uint8Array(m.W*m.H);
    if(!s.layers||width<5)return;
    // Map-fixed resistant outcrops, not reroll-dependent random rubble. A 2+ tile
    // core remains above two channels; the optional layer stack supplies hardness.
    for(let gy=12;gy<m.H-8;gy+=24)for(let gx=12;gx<m.W-8;gx+=24){
      const h=mixSeed(geology^Math.imul(gx,73856093)^Math.imul(gy,19349663));
      if(h%100>48)continue;
      const x=gx+((h>>>8)%9)-4,y=gy+((h>>>16)%9)-4,radius=1.6+((h>>>24)%6)*.2;
      const i=y*m.W+x,level=m.heights[i],hard=s.layers?(m.rockLayers?.[level]??(((level+geology%4)%4===0)?1:0)):0;
      // Broad competent rock masses may retain a core even between hard lips.
      if((!hard&&h%3!==0)||level<3||m.water.depth[i]>.1)continue;
      if(Math.hypot(x-origin%m.W,y-Math.floor(origin/m.W))<this.radius*2+radius+7)continue;
      if(end!==undefined&&Math.hypot(x-end%m.W,y-Math.floor(end/m.W))<this.radius+radius+5)continue;
      this.knobs.push({x,y,radius});
      for(let yy=Math.ceil(y-radius);yy<=Math.floor(y+radius);yy++)for(let xx=Math.ceil(x-radius);xx<=Math.floor(x+radius);xx++)
        if(xx>=0&&yy>=0&&xx<m.W&&yy<m.H&&Math.hypot(xx-x,yy-y)<=radius)this.rock[yy*m.W+xx]=1;
    }
  }
  width(distance:number):number {
    // Smooth reaches at two scales, plus a short constriction preceding rapids.
    const reach=.99+.24*Math.sin(distance*.105+this.phase)+.13*Math.sin(distance*.037+this.phase2);
    const throat=1-.17*Math.pow(Math.max(0,Math.sin(distance*.19+this.phase2)),6);
    return Math.max(1.05,this.radius*reach*throat);
  }
  swing(progress:number,available=Infinity):number {
    // Width and wavelength are distances. All Wander values share the same
    // navigation turn limit; none integrates another turn into its guide.
    const amplitude=Math.min(.7+this.wander*(8+this.radius*5),available*.3);
    const wavelength=95*(1-this.wander)+this.wander*(25+this.radius*3);
    const k=2*Math.PI/wavelength,phase=k*progress+this.phase;
    return Math.atan(amplitude*k*(Math.cos(phase)+.12*Math.cos(phase*.5+this.phase2)));
  }
  /** A seeded sequence of pools, single-level rapids and occasional two-level falls. */
  grade(distance:number,power:number):number {
    const first=13+(this.seed%13),spacing=15+(mixSeed(this.seed)%14);
    if(distance<first)return 0;
    const reach=Math.floor((distance-first)/spacing);
    let drop=0;
    for(let n=0;n<=reach;n++)drop+=power>40&&mixSeed(this.seed+n*97)%4===0?2:1;
    return drop;
  }
  lanes(x:number,y:number,dx:number,dy:number,width:number):{lanes:Lane[];knob?:Knob}{
    let selected:Knob|undefined,best=Infinity;
    const length=width*1.2+7;
    for(const k of this.knobs){
      const vx=k.x-x,vy=k.y-y,along=vx*dx+vy*dy,side=-vx*dy+vy*dx;
      if(Math.abs(along)>length||Math.abs(side)>width+k.radius+2)continue;
      const d=Math.hypot(vx,vy);if(d<best){best=d;selected=k;}
    }
    if(!selected)return {lanes:[{x,y,width}]};
    const along=(selected.x-x)*dx+(selected.y-y)*dy,side=-(selected.x-x)*dy+(selected.y-y)*dx;
    const t=Math.max(0,1-Math.abs(along)/length),ease=t*t*(3-2*t);
    const branchWidth=Math.max(1.05,width*(1-.56*ease)),spread=(selected.radius+branchWidth+.9)*Math.sin(t*Math.PI/2);
    return {knob:selected,lanes:[-1,1].map(sign=>{
      const offset=side*ease+spread*sign;return {x:x-dy*offset,y:y+dx*offset,width:branchWidth};
    })};
  }
}
