...
import { plainEntities, type ForceMap as QuakeMap } from '../core/map';
export const MAPS=[
...
export async function loadMap(id:string):Promise<QuakeMap>{
 ...
 if(kind==='seed'){
  ...
  return {name:MAPS.find(a=>a[0]===id)?.[1]??id,W:b.W,H:b.H,heights:b.heights,entities:plainEntities(b.entities),water:{depth:b.water,contamination:b.contamination},maxHeight:22,rockLayers:geology(b.heights),fallen:[],lava:new Uint32Array(b.heights.length)};
 }
...
export function placeMap(bytes:Uint8Array):QuakeMap{
 ...
 return {name:p.name,W:p.W,H:p.H,heights,entities:plainEntities(placeEntities(p,heights)),water:{depth:new Float64Array(heights.length),contamination:new Float64Array(heights.length)},maxHeight:22,rockLayers:geology(heights),fallen:[],lava:new Uint32Array(heights.length)};
}
...
export function fixture(kind='river',W=128):QuakeMap{
 ...
 return {name:'Study · '+kind,W,H,heights,entities:plainEntities(entities),water:{depth,contamination:new Float64Array(W*H)},maxHeight:22,rockLayers:geology(heights),fallen:[],lava:new Uint32Array(heights.length)};
}
