import { ImpactPlan, DEFAULTS, snapshot, waterRun, protectedGround, type CraterMap, type Settings, type Intent } from './engine';
import { loadMap } from './maps';
import { operation, applyOperation, type CraterOperation } from './operation';
...
let map:CraterMap,last:CraterMap|null=null,epoch=0,busy=false,active=false;
let before:CraterMap,impactBase:CraterMap,plan:ImpactPlan|null=null,settings:Settings={...DEFAULTS},intent:Intent={origin:0};
interface Entry {op:CraterOperation;base:CraterMap;intent:Intent;settings:Settings;nextSeed:number}
...
async function frame(token:number,reset=false,transition?:CraterMap,captureBefore=false){
  ...
  send({type:'frame',heights:map.heights,protected:protectedGround(map),undo:undo.length,redo:redo.length,canReroll:undo.length>0,name:map.name,captureBefore});
}
async function settle(token:number,live=false){
  ...
  while(!result){
    ...
    if(live&&run.ticks%128===0&&!result){
      map.water={depth:run.sim.D.slice(),contamination:run.sim.C.slice()};
      send({type:'status',text:'Water finding its level…'});await frame(token);
    }
  }
  check(token);map.water={depth:run.sim.D.slice(),contamination:run.sim.C.slice()};return result;
}
function storedMap(raw:any):CraterMap{
  const N=raw?.W*raw?.H;
  if(!Number.isInteger(raw?.W)||!Number.isInteger(raw?.H)||raw.W<4||raw.W>256||raw.H<4||raw.H>256||
    ...
    !Array.isArray(raw.entities)||!Array.isArray(raw.fallen)||raw.rockLayers?.length!==23)throw Error('Invalid saved land');
  return {...raw,maxHeight:22,heights:Uint8Array.from(raw.heights),water:{depth:Float64Array.from(raw.water.depth),contamination:Float64Array.from(raw.water.contamination)}};
}
self.onmessage=async(event:MessageEvent)=>{
  ...
  try{
    if(active&&!['finish','snapshot'].includes(msg.type))throw Error('Let this impact finish first');
    switch(msg.type){
      case 'load':
      ...
      case 'reroll':{
        ...
        if(reroll&&!series)throw Error('Make an impact first');
        before=snapshot(map);impactBase=reroll?series!.base:before;
        ...
        const next=new ImpactPlan(impactBase,settings,intent);plan=next;active=true;
        send({type:'started',settings,intent,anatomy:plan.anatomy});
        while(!next.advance(4)){await yieldSlice();check(token);}
        check(token);map=next.map;
        await frame(token,false,before);check(token);
      ...
      case 'finish':{
        ...
        const entry:Entry={op,base:impactBase,intent:{...intent},settings:{...settings},nextSeed:settings.seed};
        await frame(token);check(token);
        undo.push(entry);redo=[];if(undo.length>16)undo.shift();active=false;plan=null;
        send({type:'operation',op,base:before,impactBase});
        send({type:'finished',...result,seed:settings.seed,undo:undo.length});break;
      }
      case 'undo':
      ...
      case 'replay':{
        ...
        const base=storedMap(b.base),original=storedMap(b.impactBase??b.base),op=b.operation as CraterOperation;
        const restored=applyOperation(base,op);
        map=base;before=base;impactBase=original;undo=[];redo=[];last=null;
        await frame(token,true,undefined,true);
        ...
        send({type:'operation',op,base,impactBase:original});
