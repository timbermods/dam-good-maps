...
import type { CraterMap } from './engine';
...
export function frameContext(m: CraterMap) {
  const view = waterFromDepth(m.heights, m.water.depth, m.water.contamination);
  return { view, surface: surfaceWater(m.W,m.H,view) };
}
export function makeChunk(m: CraterMap, cx: number, cy: number, context: ReturnType<typeof frameContext>, includeObjects: boolean, before?:CraterMap): Chunk {
...
export function changedChunks(m:CraterMap,old:CraterMap|null): {cx:number;cy:number;objects:boolean}[] {
...
function transitionMesh(m:CraterMap,old:CraterMap,cx:number,cy:number):Geometry {
