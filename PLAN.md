...
| D194 | **Carve becomes a force of nature** (amends D180 (6), D181 (1) and D184's top bar). Codex is redesigning the carving prototype on `investigation/carve` (PR #47): Carve has Unleash and Aim modes, Defy gravity, and a Power slider from creek to catastrophe, and gets its own top-bar button next to Source. PR #47 is held: it is not merged at the next boundary, only once Kyler says it is ready; Live editing's carving waits for it. | Kyler: carving should feel like unleashing a force of nature. | Kyler's decision, 2026-09-26; Kyler, 2026-09-26: he loves #47; it stays held for one more Codex round (Wander, Width separate from Power, variation within each carve, "Try another path") until he says it's ready to merge; then it becomes the Carve button in Live editing, next to Source; Kyler, 2026-09-26: #47 is ready (he tried the final demo): the hold is released, it merges at the next boundary, and it is the starting point for the Carve button in Live editing, keeping its full feature set (D199) |
...
| D211 | **M9a's two settings questions.** (1) Lake Basin's water share: the settings check is information until M9b fixes Lake Basin's water, as design version 2 planned. (2) Start area: the setting leans the land toward roomier or tighter benches and the start prefers a matching one; no stamped bench (D209). It is made honest: renamed or described as a preference ("prefer a roomy start" / "prefer a tight start"), and the map card shows the actual bench size. The editor's Flatten, with its "the start fits here" hint, is how a player makes a bigger bench by hand. | M9a's generator can't guarantee either without breaking D209. | Kyler's decision, 2026-09-26 |
| D212 | **Live editing: two changes, then release.** Kyler: it feels amazing. (1) Sources move from the top bar to the left shelf as two separate items right after Start: "Water source" and "Badwater source" (simpler than one item with a switch); the shelf reads Start, Water source, Badwater source, Pine, Birch, Oak, Berry bush and so on; the top bar keeps only the shaping tools, the forces and Remove; selecting a placed source still shows its strength control. (2) Transparent water: only water under or right around the brush turns clear, and only while the brush is over tiles that already have water (painting a submerged bed); working on dry land (flattening land between two rivers to join them, say) leaves the water normal so it can be seen. When clear, water still reads as water: a faint blue tint, its ripples and a soft bright shoreline outline (today it looks like pale grey glass). T still toggles clear water for the whole map. (3) Defaults confirmed: the R/F camera zoom is gone (F resizes, R turns objects); juice sounds on by default but quiet, with the off switch; layers on Alt+middle-click and Alt+click. Then released as `live-editing-done`. | Kyler tried the four pushes. | Kyler's decision, 2026-09-26 |
| D213 | **Badwater (#54) goes in with M9a.** #54 is held and M9a takes it in (M9a has merged it). When removing the last badwater spring would leave none, it isn't refused: the map switches to "No badwater" and says so in a quiet line (Claude's P09 too). | Kyler's answer to #54 and P09. | Kyler's decision, 2026-09-26 |
| D214 | **Real places: rivers, not floods.** No water sources at 8× the official strength (that makes floods, not rivers): strengths stay near the official range; the start moves closer to water instead (as Pick a place's designed water does); any place that still can't work is dropped. The "Centre" suffixes are replaced with names from a real feature or direction (for example "North Rim", "Upper Valley"). Kyler sends the places to drop from the sheet later. | Kyler's review of the rebuilt gallery. | Kyler's decision, 2026-09-26 |
| D215 | **Waterfalls (#53) before release:** remove the V-shaped gap that splits falls into two ribbons (the sheet must be continuous), and add more whitewater and splash where the fall lands; then released as `look-waterfalls-done` without another review unless it looks off. | Kyler's review of the captures. | Kyler's decision, 2026-09-26 |
| D216 | **The forces: three ready, Quake held.** Carve (#47) is ready: Codex added both touches (bends vary in width and depth; oxbows hold water behind sediment bars; an unfed oxbow evaporating over time is correct game physics); merged and built as the Carve button with its full feature set (D199). Craterize (#51) is ready (the rays read as an organic starburst; Steep and Terraced walls are distinct); merged and built as the Craterize button. Erupt (#50) is ready (Steep and Broad differ, lava lobes wind, the eruption swells, plumes and cools); merged and built as the Erupt button, with the plume billowing bigger and darker at high power. Quake (#52) stays held: Lift is great, Slide gets one more Codex round (moving far, on every stroke); if Slide can't be made dramatic, Quake becomes Lift only; Kyler says when it's ready. All four are built on one shared forces core, in a visually distinct forces group on the top bar, each options row starting with its mode switch (D203, D206). | Kyler tried the prototypes. | Kyler's decision, 2026-09-26 |
| D217 | **3D carving is smarter Lower and Raise,** not new Carve and Fill buttons (Carve is the river force): Lower aimed at a cliff face digs into it; Raise with a layer selected builds in the air. **M12's mapping** from the docs sweep is accepted, as long as it follows D139 (Claude steers the generator for character, and uses editor operations only for precise edits). #66 (the candidate intentions) stays for later. | Kyler's answers to the docs sweep's two readings. | Kyler's decision, 2026-09-26 |
| D218 | **Probe batches on the dedicated machine** (an exception to D117, on one computer only). The milestone session moved to a computer kept for this work, where nobody plays. There, the DGM Probe may run a batch whenever the plan calls for one, without asking first. Everything else in the probe rule stands: never while Timberborn is running or other heavy work runs; the mod installed only while a batch runs; the game's settings backed up and verified afterwards; saves and mods untouched; results in `C:\dgm-probe`. Every batch and its results are reported in `docs/STATUS.md` and the progress log issue (D221). On any other computer, D117 applies unchanged. Releases still need Kyler's approval. | Kyler is away while the work continues. | Kyler's decision, 2026-09-26 |
| D219 | **Quake is ready, with both Lift and Slide** (amends D216). Codex's Slide round is done (#52 at a293e41). #52 merges with #47, #51 and #50, and all four forces are built as top-bar buttons in the visually distinct forces group, on one shared forces core, each options row starting with its mode switch. The forces go to the preview; they are released only after Kyler has tried them. | Kyler, 2026-09-26: all four are ready. | Kyler's decision, 2026-09-26 |
| D220 | **Codex's forces-core and juice investigations** (`investigation/forces-core`, one shared core for the four forces; `investigation/juice`, procedurally synthesised editor sounds): each is merged at a boundary once green, and adopted as proposals. If the forces core has landed, the forces are built on it. The juice sounds are hooked into Live editing (on by default, quiet, with an off switch, as D212 confirmed) and shown on the preview. | Kyler's brief for the work while he is away. | Kyler's decision, 2026-09-26 |
| D221 | **A "Progress log" issue** ([#57](https://github.com/timbermods/dam-good-maps/issues/57)): the milestone session adds a short, plain comment each time it finishes a step, releases something, runs a probe batch or parks something for Kyler (what happened, links, what's next). `docs/STATUS.md` stays the full record. | Kyler wants a brief running log. | Kyler's decision, 2026-09-26 |
